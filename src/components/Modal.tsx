...
export default function Modal({ setShowModal, data }: ModalProps) {
  ...
  return (
    <motion.div
      ...
      className="absolute top-0 right-0 h-full px-4 pt-4 pb-8 sm:w-full md:w-[500px] bg-black/50 backdrop-blur-md z-50"
    >
      <button
      ...
      <div className="flex flex-col h-full">
        <h1 className="text-white sm:text-[5px] md:text-3xl mb-5 font-bold uppercase">
          {data?.title}
        </h1>
        <span className="text-[#e6e6e6] mb-12">{data?.date}</span>
        <p className="text-[#cecece] md:text-lg first-letter:uppercase">
          {data?.explanation}
        </p>
      </div>
    </motion.div>
